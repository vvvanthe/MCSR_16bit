...
TRUE_IMAGE_DIR='true'
QP=42
dataset='BVI_{}/'.format(QP)
full_data='dataset'+"/"+dataset
batch_dir='batch_data'+"/"+dataset

in_image_dir='dataset/BVI-DVC-BICUBIC_{}/'.format(QP)
tr_image_dir='dataset/BVI-DVC-ORG-F1/'
...
def build_image_set(file_truepath,scale,convert_ycbcr=False,resampling_method="bicubic",print_console=True):
    ...
    w,h,_=true_image.shape
    if true_image.shape[2] == 3:
        name = os.path.basename(file_truepath)
        filename, extension = os.path.splitext(name)

        file_inpath=in_image_dir+ "/" +name.split('_')[0]+'_{}x{}_{}.png'.format(int(h/2),int(w/2),QP)
        ...
        if convert_ycbcr:
            input_image=util.convert_rgb_to_y(input_image,bit=16)
            print(input_image.shape)
            input_interpolated_image=util.convert_rgb_to_y(input_interpolated_image,bit=16)
            true_image=util.convert_rgb_to_y(true_image,bit=16)
...
def save_batch_image( image_number, image,type):
    return util.save_image(batch_dir + "/" + type + "/%08d.png" % image_number, image)
