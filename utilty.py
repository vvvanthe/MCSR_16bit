...
def apply_SR(model,input_image,scale,bit):

    if len(input_image.shape) >= 3 and input_image.shape[2] == 3:
        input_y_image = convert_rgb_to_y(input_image,bit=bit)
        #print(input_y_image.shape)
        slices=np.split(input_y_image, 1, axis=1)

        #c=np.concatenate([slices[0][:,-100:],slices[0][:,0:100]],axis=1)
        #print(np.shape(c))
        #slices.append(c)
        blocks=[]
        for slice in slices:

            sli=tf.image.convert_image_dtype(slice, tf.float32)/(2 ** bit - 1)
            sli = tf.expand_dims(sli, axis=0)
            output_y_image =model(sli)
            output_y_image = output_y_image.numpy()[0] * (2 ** bit - 1)
            blocks.append(output_y_image)


        #print(np.shape(blocks[0][:,:-100]))
        #print(np.shape(blocks[2][:,100:-100]))
        #print(np.shape(blocks[1][:,100:]))

        #output_y_image=np.concatenate([blocks[0][:,:-100],blocks[2][:,100:-100],blocks[1][:,100:]],axis=1)
        output_y_image = np.concatenate(blocks, axis=1)
        #output_y_image =  tf.squeeze(output_y_image)
        y_bic=convert_rgb_to_y(resize_image_by_pil(input_image, scale),bit=bit)
        #y_bic = y_bic.astype(np.float32)
        output_y_image = np.add(output_y_image, y_bic)
        # print(np.max(output_y_image))
        output_y_image = np.clip(output_y_image, 0, 2 ** bit - 1)
        #print(np.max(output_y_image))
        #output_y_image=output_y_image.astype(np.uint16)
        scaled_ycbcr_image = convert_rgb_to_ycbcr(
            resize_image_by_pil(input_image, scale),bit=bit)
        image = convert_y_and_cbcr_to_rgb(output_y_image, scaled_ycbcr_image[:, :, 1:3],bit=bit)

        image = np.clip(image, 0, 2 ** bit - 1)

    else:
        input_y_image = input_image
        input_y_image = tf.expand_dims(input_y_image, axis=0)
        input_y_image = tf.image.convert_image_dtype(input_y_image, tf.float32)
        output_y_image = model(input_y_image)
        # output_y_image =  tf.squeeze(output_y_image)
        y_bic = resize_image_by_pil(input_image, scale)
        output_y_image = np.add(output_y_image.numpy()[0] * (2**bit-1), y_bic)

        image = np.clip(output_y_image, 0, (2**bit-1))

    return image

def apply_SR_backup(model,input_image,scale,bit):

    if len(input_image.shape) >= 3 and input_image.shape[2] == 3:
        input_y_image = convert_rgb_to_y(input_image,bit=bit)
        input_y_image=tf.expand_dims(input_y_image,axis=0)
        input_y_image=tf.image.convert_image_dtype(input_y_image, tf.float32)/(2 ** bit - 1)
        output_y_image =model(input_y_image)
        #output_y_image =  tf.squeeze(output_y_image)
        y_bic=convert_rgb_to_y(resize_image_by_pil(input_image, scale),bit=bit)
        #y_bic = y_bic.astype(np.float32)
        ...
        image = np.round(output_y_image, 0, (2**bit-1))
...
def convert_rgb_to_y(image,bit):
    y_image=np.zeros([image.shape[0], image.shape[1], 1])

    if len(image.shape) <= 2 or image.shape[2] == 1:
        return image
    xform = np.array([[0.299, 0.587, 0.114]])
    y_image[:, :, 0]= cv2.cvtColor(image,cv2.COLOR_BGR2YCrCb)[:,:, 0]

    return y_image



def convert_rgb_to_ycbcr(image,bit):
    if len(image.shape) < 2 or image.shape[2] == 1:
        return image
    ...
    #ycbcr_image[:, :, 0] += 0.
    #ycbcr_image[:, :, [1, 2]] += 2**(bit-1.0)
    ycbcr_image=cv2.cvtColor(image,cv2.COLOR_BGR2YCrCb)

    return ycbcr_image


def convert_ycbcr_to_rgb(ycbcr_image,bit):
    '''
    rgb_image = np.zeros([ycbcr_image.shape[0], ycbcr_image.shape[1], 3])  # type: np.ndarray
    rgb_image[:, :, 0] = ycbcr_image[:, :, 0]
    rgb_image[:, :, [1, 2]] = ycbcr_image[:, :, [1, 2]] - 2**(bit-1.0)
    ...
    xform = np.array(
        [[1, 0, 1.402],
         ...
         [1, 1.772, 0]])
    '''
    ycbcr_image=ycbcr_image.astype(np.uint16)
    rgb_image = cv2.cvtColor(ycbcr_image,cv2.COLOR_YCrCb2BGR)

    return rgb_image

def convert_y_and_cbcr_to_rgb(y_image, cbcr_image,bit):
    ...
    return convert_ycbcr_to_rgb(ycbcr_image,bit=bit)
...
def blockDivide(img_in, block_size):
    img=set_image_alignment(img_in,block_size)
    # Define dimension of image
    dimension = img.shape[0]

    # Set number of slices per axis
    num0=int(img.shape[0]/block_size)
    num1 = int(img.shape[1] / block_size)
    blockNumber=int(num0*num1)
    ...
    split_a = np.split(img, num0, axis = 0)
    ...
    for i in range(num0):
        for j in range(num1):
            ...
            split_b = np.split(split_a[i], num1, axis = 1)
            # Fill array with blocks
            blocksArray.append(split_b[j])

            # Increase counter
            counter += 1

    return np.asarray(blocksArray)


def get_split_with_channels( image,window_size,stride):

    if image.shape[2] == 1:
        return get_split_images(image,window_size,stride)
    else:
        height, width,_ = image.shape
        red = blockDivide(np.reshape(image[:,:,0],(height,width,1)),window_size)
        green =blockDivide(np.reshape(image[:,:,1],(height,width,1)),window_size)
        blue=  blockDivide(np.reshape(image[:,:,2],(height,width,1)),window_size)

        out=tf.keras.layers.concatenate([red, green, blue])
